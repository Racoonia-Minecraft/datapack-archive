...
execute if score $out random matches 12 run tellraw @s [{"text":"“Es gibt keine größere Liebe, als wenn einer sein Leben für seine Freunde hingibt.” "},{"text":"(Johannes 15:13)", "color": "gray"}]
execute if score $out random matches 13 run tellraw @s [{"text":"“Jesus aber sah sie an und sprach zu ihnen: 'Bei den Menschen ist’s unmöglich; aber bei Gott sind alle Dinge möglich.'” "},{"text":"(Matthäus 19:26)", "color": "gray"}]
...
execute if score $out random matches 19 run tellraw @s [{"text":"“Unser Glaube kann Berge versetzen.” "},{"text":"(Matthäus 17:20)", "color": "gray"}]
